...
exports.createTodo = async (reqParams) => {
 try {
  const { title, date, userId, description, priority, taskStatus } = reqParams
  ...
  const insertObj = { title, todoDate: new Date(date), userId: mongo.getId(userId), createdAt: new Date(), status: 1, description, priority, taskStatus }
...
exports.fetchTodo = async (reqParams) => {
 try {
  ...
  const pipeline = [
   ...
   {
    $project: { title: 1, todoDate: 1, createdAt: 1, todoDateDMY: 1, description: 1, priority: 1, taskStatus: 1 }
   }
  ]
...
exports.updateTodo = async (reqParams) => {
 try {
  const { todoId, title, date, description, priority, taskStatus } = reqParams;
  const db = await mongo.getDB();
  const updateObj = {
   title, todoDate: new Date(date), updatedAt: new Date(), description, priority, taskStatus
  }
